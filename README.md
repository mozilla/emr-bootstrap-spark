...
aws emr create-cluster \
  ...
  --bootstrap-actions Path=s3://support.elasticmapreduce/spark/install-spark,Args=["-v","1.3.1.e"] Path=s3://${SPARK_BUCKET}/telemetry.sh Path=s3://elasticmapreduce/bootstrap-actions/configure-hadoop,Args=["-y","yarn.nodemanager.vmem-check-enabled=false","-y","yarn.nodemanager.pmem-check-enabled=false"]
```
